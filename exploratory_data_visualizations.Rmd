...
jfillc <- scale_fill_continuous(low=BLUE, high=RED, na.value="#cfcfcf")
...
ggplot(data = wv) +
  geom_sf(aes(fill=cor_od_food, label=ifelse(FIPS > 54108 & FIPS < 54110, "Wyoming County", NA)), size=0.05) + 
...
drops <- c("FIPS","Group.1", "Group.2", "State", "County")
...
x %>% 
  ...
  ggplot(aes(x = rowname, y = `Drug Overdose Mortality Rate`, fill=posneg)) +
    ...
    labs(x="WEST VIRGINIA HEALTH STATISTIC",
         ...
         title="FOOD INSECURITY AND PREVENTABLE HOSPITAL VISITS\n ARE MOST ",
         caption="SOURCE: WEST VIRGINIA DEPT. OF HHR") +
    jtheme + jfill +
    theme(
      panel.grid.major.x = element_blank(),
...
drops <- c("% African American", "% American Indian/Alaskan Native", "% Asian", "% Hispanic", "% Limited Access", "% Native Hawaiian/Other Pacific Islander",
           "% Non-Hispanic White")
wv_no_race <- wv_stats[ , !(names(wv_stats) %in% drops)]
...
wv_no_race %>%
  ...
  ggplot(aes(x = value, y = `Drug Overdose Mortality Rate`)) +
    ...
    stat_smooth(fill=SKY, color=BLUE) + 
  scale_x_continuous(limits=c(min(value), max(value)) +
  jtheme +
  theme(
    strip.background=element_blank(),
    strip.text=element_text(family="Quick", face="bold")
  )
